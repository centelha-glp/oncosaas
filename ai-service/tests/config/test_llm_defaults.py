...
def test_merge_tenant_overrides_env(monkeypatch):
    ...
    monkeypatch.setattr(main.settings, "llm_orchestrator_model", "claude-haiku-4-5", raising=False)
    monkeypatch.setattr(main.settings, "llm_subagent_model", "claude-haiku-4-5", raising=False)
    ...
    assert merged["orchestrator_model"] == "claude-haiku-4-5"
...
def test_merge_respects_has_anthropic_when_provider_unset(monkeypatch):
    ...
    assert anthropic["llm_model"] == "claude-haiku-4-5"
