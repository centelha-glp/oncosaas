...
describe('applyComplementaryExamsFromAiItems', () => {
  ...
  it('mantém creatinina e eTFG separados no mesmo performedAt', async () => {
    const { tx, exams, results } = makeTx();
    const items: AiComplementaryExamItem[] = [
      {
        type: 'LABORATORY',
  ...
  it('não mistura creatinina com painel eTFG no mesmo exame', async () => {
    const { tx, exams, results } = makeTx();
    await applyComplementaryExamsFromAiItems(
      tx as unknown as Prisma.TransactionClient,
      baseArgs,
    ...
    expect(exams).toHaveLength(2);
    expect(results).toHaveLength(2);
    expect(results.map((r) => r.valueNumeric)).toEqual([1.1, 72]);
  });
